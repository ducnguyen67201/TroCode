...
export function ClassroomLessonDesktopControls({ state, vi }: { state: LessonLocalState; vi: boolean }) {
  const [windows, setWindows] = useState<{ token: string; label: string }[]>([]);
  const [error, setError] = useState('');
  const [busy, setBusy] = useState(false);
  const idle = ['paused', 'blocked', 'waiting_for_student'].includes(state.status);
  useEffect(() => { setWindows([]); }, [state.envelope.lessonId, state.revision]);
  ...
  return <div className="lesson-desktop-controls">
    ...
    {idle && <>
      ...
      <button disabled={busy} type="button" onClick={() => void work(async () => setWindows(await window.tro.lessons!.windows({ lessonId: state.envelope.lessonId, revision: state.revision })))}>{vi ? 'Chọn cửa sổ tài liệu' : 'Choose material window'}</button>
      {windows.map((choice) => <button key={choice.token} disabled={busy} type="button" onClick={() => void work(async () => { await window.tro.lessons!.selectWindow({ lessonId: state.envelope.lessonId, revision: state.revision, token: choice.token }); setWindows([]); })}>{choice.label}</button>)}
