...
import { requestTaskInteraction } from './main/application/request-task-interaction';
...
import { boundsEqual, pointEqual } from './main/companion/overlay-geometry';
...
} from './main/knowledge/classroom-lesson-agent-tools';
import { lessonCompanionCard } from './main/knowledge/classroom-lesson-companion';
import {
  ...
  additionalToolAdapters: [
    ...
    ...createWorkspaceRuntimeToolAdapters(),
    {
      ...
      execute: async (invocation, context) => {
        ...
        const answer = await requestTaskInteraction(taskRuntime, context.taskId, input);
        return {
...
const coachRuntime: CoachRuntime = new CoachRuntime({
  ...coachDecisionClient,
  beforeLessonRound: async (taskId, signal) => { if (classroomLessons.runner.has(taskId)) { signal.throwIfAborted(); await classroomLessons.controller.authorize(); } },
  ...
  onStatus: (taskId, phase, summary) => {
    ...
    if (!classroomLessons.runner.has(taskId)) classroomGuidanceCoordinator.onStatus(taskId, phase, summary);
    agentActivityService.publish(taskId, { kind: 'status', summary });
  },
  onTerminal: async (taskId, terminal) => {
    if (classroomLessons.runner.has(taskId)) await classroomLessons.runner.terminal(taskId, terminal);
    taskRuntime.complete(taskId, terminal);
    if (!classroomLessons.runner.has(taskId)) void classroomGuidanceCoordinator.onTerminal(taskId, terminal);
...
const localAgentRuntime = new LocalAgentRuntime({
  ...
  onTerminal: async (terminal) => {
    if (classroomLessons.runner.has(terminal.threadId)) await classroomLessons.runner.terminal(terminal.threadId, terminal);
...
const taskApplicationService: TaskApplicationService = new TaskApplicationService(
  ...
  onTaskCancelled: (taskId) => { classroomGuidanceCoordinator.onTaskCancelled(taskId); classroomLessons.runner.onTaskCancelled(taskId); classroomLessons.controller.onChildCancelled(taskId); },
...
const classroomLessons = createClassroomLessonFeatures(lessonServices, {
  tasks: taskApplicationService, cua: cuaService, knowledge: knowledgeSpaceClient, session: classroomSessionService,
  owner: async () => (await authService.assertSignedIn()).id,
...
function prepareApplicationShutdown(): Promise<void> {
  ...
  classroomLessons.feed.stop();
  const classroomGuidanceShutdown = Promise.all([classroomGuidanceCoordinator.shutdown(), classroomLessons.controller.shutdown()]);
...
function activateGlobalCompanionResponseShortcuts(
  ...
  if (response.lesson) { globalNumberedChoiceShortcuts?.deactivate(); return; }
  if (
    !auxiliaryWindowsEnabled ||
    response.phase !== 'completed' ||
...
const createWindow = (): void => {
  ...
  unregisterIpcHandlers = registerIpcHandlers(nextMainWindow, {
    ...
    classroomLessonFeatures: classroomLessons,
    ...
    onAuthSignedOut: async () => {
      teacherClassroomContextService.clear();
      classroomLessons.feed.stop();
      await classroomLessons.controller.activate(null, false);
