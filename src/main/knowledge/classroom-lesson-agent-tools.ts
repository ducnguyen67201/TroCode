...
export function lessonToolDefinitions(): RuntimeToolDefinition[] {
  return [
    ...
    },
    {
      ...
      operations: ['prepare'],
      description:
        'Prepare a teacher-reviewed lesson preview. List session assignments and obtain lesson context first. planJson is a strict ClassroomLessonPlan semantic plan; no tool commands. When context.maxPlanVersion >= 3, use schemaVersion 3: every step needs surface {kind: resource_app, navigation: student} for a class source or web material. This downloads and opens the original on each student computer. Use current_screen with current_window only when the teacher explicitly asks to explain what students already have open. No native paths or commands belong in plans. Fall back to schemaVersion 1 for explain/demonstrate/practice/check on older servers. For opening material without explanation, use schemaVersion 2 and an open step with no criterionIds, only when context.maxPlanVersion >= 2. Explain already opens its material; do not add explanation to an open-only request. This cannot broadcast. The teacher must review and confirm in the lesson panel.',
      available: (context) => Boolean(context?.teacherClassroom) && !context?.activity,
