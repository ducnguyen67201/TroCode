...
class MemoryAgentTurnRepository {
  ...
  async create(input) {
    ...
    const denial = input.authorize({ weekMessages: this.weekMessages });
...
test('Free reserves at most 25 weekly messages', async () => {
  const repository = new MemoryAgentTurnRepository(24);
  const service = new AgentTurnService(repository, { mode: 'enforce' });
  const freeRequest = (clientTurnId) => ({
    ...request(clientTurnId),
    planId: 'free',
  });

  await service.create(
    freeRequest('22222222-2222-4222-8222-222222222222'),
  );
  assert.equal(repository.weekMessages, 25);

  await assert.rejects(
    service.create(
      freeRequest('33333333-3333-4333-8333-333333333333'),
    ),
    (error) => error.code === 'weekly_message_limit_reached',
  );
});

test('Basic reserves exactly one weekly message per user turn', async () => {
  const repository = new MemoryAgentTurnRepository(299);
  const service = new AgentTurnService(repository, { mode: 'enforce' });
  ...
  assert.equal(repository.weekMessages, 300);
...
test('repeating a client turn ID is idempotent and does not consume another message', async () => {
  ...
  assert.equal(repository.weekMessages, 1);
});
