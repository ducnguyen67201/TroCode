...
export class PostgresUsageRepository {
      ...
      const inserted = await client.query(
        `INSERT INTO model_budget_reservations
           (request_id, user_id, task_id, lane, model, catalog_version,
            reserved_micro_usd, status, would_deny)
  ...
  async markDispatched(userId, requestId) {
    return this.transition(userId, requestId, {
  ...
  async transition(userId, requestId, transition) {
    const result = await this.pool.query(
    ...
    if (result.rows[0]) return normalizeReservation(result.rows[0]);
